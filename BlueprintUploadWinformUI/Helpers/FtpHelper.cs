    ...
    public class FtpHelper
    {
        ...
        public string RemotePath
        {
            ...
            set
            {
                ...
                this.remotePath = "/" + value.TrimStart('/').TrimEnd('/') + "/";
            }
        }
        ...
        public FtpHelper(string host, string username, string password) : this(host, username, password, defaultFtpPort, null, false, true, false, defaultReadWriteTimeout)
        {
        }
        ...
        public bool Upload(FileInfo localFile, string remoteFileName, ProgressBar prgPercent, double filePercent)
        {
            bool result = false;
            if (localFile.Exists)
            {
                try
                {
                    ...
                    using (FileStream fs = localFile.OpenRead())
                    {
                        byte[] buffer = new byte[1024 * 4];
                        long current = 0;
                        long totle = localFile.Length;
                        int count = fs.Read(buffer, 0, buffer.Length);
                        while (count > 0)
                        {
                            current += count;
                            prgPercent.Value = Convert.ToInt32(100 * filePercent * current / totle);

                            rs.Write(buffer, 0, count);
                            count = fs.Read(buffer, 0, buffer.Length);
                        }

                        prgPercent.Value = Convert.ToInt32(100 * filePercent);
        ...
        public bool UploadMultiFile(List<string> filePaths, ProgressBar prgPercent, ListView lvFileInfo)
        {
            long totleLength = 0, currentLength = 0;
            bool result = true;
            ...
            filePaths.ForEach(item =>
            {
                ...
                totleLength += fileInfo.Length;
                localFiles.Add(fileInfo);
            });

            for (int i = 0; i < localFiles.Count; i++)
            {
                FileInfo item = localFiles[i];
                lvFileInfo.Items[i].SubItems[2].Text = UpLoadStateEnum.上传中.ToString();

                try
                {
                    string url = UrlCombine(Host, RemotePath, item.Name);
                    FtpWebRequest request = CreateConnection(url, WebRequestMethods.Ftp.UploadFile);
                    ...
                    using (Stream rs = request.GetRequestStream())
                    {
                        using (FileStream fs = item.OpenRead())
                        {
                            ...
                            {
                                currentLength += count;
                                double percent = (double)currentLength * 100 / totleLength;
                                prgPercent.Value = Convert.ToInt32(percent > 100 ? 100 : percent);
                    ...
                    lvFileInfo.Items[i].BackColor = Color.FromArgb(108, 226, 108);
                }
                catch
                {
                    result = false;
                    ...
                    lvFileInfo.Items[i].BackColor = Color.FromArgb(240, 73, 8);
