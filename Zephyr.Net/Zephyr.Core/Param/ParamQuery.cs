...
namespace Zephyr.Core
{
    public class ParamQuery
    {
        ...
        public ParamQuery Union(string sql)
        {
            data.Union = sql;
            return this;
        }
        ...
        public ParamQuery ClearWhere(string column,bool clearFlag, params object[] extend)
        {
            if (clearFlag)
            {
                data.Where.ForEach(c =>
                {
                    if (c.Data.Column == column)
                    {
                        data.Where.Remove(c);
                    }
                }); 
