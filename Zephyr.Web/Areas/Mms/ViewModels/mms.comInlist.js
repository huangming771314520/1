...
mms.com.selectMaterial = function (vm, param) {
    ...
    opt.onSelect = function (data) {                //可接收选择数据
        ...
        var rows = data.rows;
        // var object_id = defaultRow.object_id;
        //for (var i in rows) {
        //    if (!fnExist(rows[i]))
        //        addnew($.extend({ object_id: Object_ID },grid, defaultRow, rows[i]));
        //}
        com.ajax({
            type: 'GET',
            url: url ,
            data: {id:total, BillNo: Object_ID,object_id:Object_ID },
            success: function (d) {
                var ids = d.split(',');
                for (var i in rows) {
                    if (!fnExist(rows[i]))
