...
mms.viewModel.edit = function (data) {
    ...
    this.scrollKeys = ko.mapping.fromJS(data.dataSource.pageData.scrollKeys);       //数据滚动按钮（上一条下一条）
    this.form = ko.mapping.fromJS(data.form || data.dataSource.defaultForm); //表单数据
    this.setting = data.dataSource.setting;
    this.defaultRow = data.dataSource.defaultRow;                          //默认grid0行的值
    this.defaultForm = data.dataSource.defaultForm;                        //主表的默认值
    ...
    this.gridEdit = new com.editGridViewModel(self.grid0);
    this.gridEdit.OnAfterCreateEditor = function (editors) {
        com.readOnlyHandler('input')(editors.ID.target, true);
    };
    this.grid0.onClickRow = function () { //this.grid0.onDblClickRow
        if (self.readonly()) return;
        self.gridEdit.begin();
    };
    this.grid0.toolbar = "#gridtb0";
    ...
    this.scrollTo = function (id) {
        ...
        self.grid0.url(self.urls.getdetail + id);
        self.grid0.datagrid('loaded');
    };
    this.saveClick = function () {
        if (self.readonly()) return;
        self.gridEdit.ended(); //结束grid0编辑状态
        var post = {           //传递到后台的数据
