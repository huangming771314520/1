...
order by A.MenuSeq,A.MenuCode";
        ...
        {
            //增加按钮事件名称字段ButtonClickCode
            var sql = String.Format(@"
select A.*,B.ButtonClickCode,
        ...
        public void SaveMenuButtons(string MenuCode, JToken ButtonList)
        {
            ...
            Logger("设置菜单按钮", () =>
            {
                ...
                foreach (JToken item in ButtonList.Children())
                {
                    ...
                    var ButtonClickCode = (item["ButtonClickCode"] != null && item["ButtonClickCode"].ToString() != "") ? item["ButtonClickCode"].ToString() : ("click:" + ButtonCode + "Click");//获取按钮事件Code
                    db.Insert("sys_menuButtonMap").Column("MenuCode", MenuCode).Column("ButtonCode", ButtonCode).Column("ButtonClickCode", ButtonClickCode.Replace("'", "''")).Execute();
                }
                db.Commit();
            }, e => db.Rollback());
        }
...
order by ButtonSeq";
        ...
        public List<sys_button_extend> GetCurrentUserMenuButtons_extend()
        {
            var MenuCode = GetCurrentMenuCode();
            var UserCode = FormsAuth.GetUserData().UserCode;
            var sql = @"
select A.*,C.ButtonClickCode
from sys_button A
inner join sys_roleMenuButtonMap B on B.MenuCode = @0 and B.ButtonCode = A.ButtonCode
inner join sys_menuButtonMap C on A.ButtonCode = C.ButtonCode and C.MenuCode = @0
where RoleCode in (
select RoleCode
from sys_userRoleMap
where userCode = @1
...
order by ButtonSeq";
            var result = db.Sql(sql, MenuCode, UserCode).QueryMany<sys_button_extend>();
            return result;
        }
    }
