...
where UserCode = @0 "
        ...
        public void AppendLoginHistory(JObject request)
        {
            ...
            item.HostIP = IP;
            item.LoginCity = City;
            item.LoginDate = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
        ...
        public bool AuthorizeUserMenu(List<string> urls)
        {
            ...
            string  excuteSql=string.Format(@"
            select 1
            from sys_roleMenuMap A
            left join sys_userRoleMap B on B.RoleCode = A.RoleCode
            left join sys_menu C on C.MenuCode = A.MenuCode
            where B.UserCode = '{1}'
            and C.URL in ('{0}')",string.Join("','",urls),UserCode);
            var result = db.Sql(excuteSql).QueryMany<int>();
...
left join sys_userRoleMap B on B.RoleCode = A.RoleCode and B.UserCode = '{0}'", user);
        ...
        public int EditUserPassword(string UserCode,string pwd)
        {
            var result = db.Update("sys_user")
                .Column("Password", pwd)
                .Where("UserCode", UserCode)
