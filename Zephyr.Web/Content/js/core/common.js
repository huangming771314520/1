...
com.UploadFileJS = function() {
    var self = this;
    var params = JSON.stringify(self).replace(/"/g, "'");
    return '<a href="javascript:void(0)" onclick="com.UploadFileClick.call(' + params + ')">上传附件</a>';
}

com.UploadFileClick = function() {
    var BindTableName = this.BindTableName;
    var BindCode = this.BindCode;
    ...
    opt.content = "<iframe src='/mms/COM_UploadFileManage/Index' style='height:100%;width:100%;border:0;' frameborder='0'></iframe>";
    opt.params = { BindTableName: BindTableName, BindCode: BindCode };;
    target.window(opt);
}

com.DownLoadFileClick = function() {
    var BindTableName = this.BindTableName;
    var BindCode = this.BindCode;
    var FileType = this.FileType;
    var result = $.ajax({ type: 'get', url: '/api/Mms/COM_UploadFileManage/GetFileUrl', data: { BindTableName: BindTableName, BindCode: BindCode, FileType: FileType }, async: false, cache: true });

    if (result.responseText) {
        var item = JSON.parse(result.responseText);
        if (item[0]) {
            var FileAddress = '/' + (item[0].FileAddress.substr(item[0].FileAddress.indexOf('Upload'))).replace(/\\/g, "/");
            return '<a style="text-decoration:underline;color:blue;" download="' + item[0].FileName + '" href="' + FileAddress + '">' + item[0].FileName + '</a>';
        }
        else {
    ...
    console.log(JSON.parse(result.responseText)[0]);
    //
}

com.formatCombobox = function(codetype, value) {
    htmlobj = $.ajax({ type: 'get', url: '/Commons/GetComboboxFormatter', data: { CodeType: codetype, ComboboxValue: value }, async: false, cache: true });
    return htmlobj.responseText;
}

com.formatComboboxSingle = function(tablename, idfield, textfield, value, connName) {
    var value2 = value || '';
    if (value2 == '') { return; };
    htmlobj = $.ajax({ type: 'get', url: '/Commons/GetComboboxFormatter_Single', data: { TableName: tablename, IdField: idfield, TextField: textfield, ComboboxValue: value, connName: connName }, async: false, cache: true });
    return htmlobj.responseText;
}
...
com.formatMoney = function(value) {
    var sign = value < 0 ? '-' : '';
    return sign + utils.formatNumber(Math.abs(value), '#,##0.000');
};
...
com.message = function(type, message, callback) {
    switch (type) {
        ...
        case "longerror":
            if (parent == window) return alert(message);
            var opt = { html: message, title: "错误提示" }
            parent.com.dialog(opt);
            break;
        case "warningAlert":
            if (parent == window) return alert(message);
            parent.$.messager.alert('消息提示', message);
            break;
    }
    ...
    this.getCheckRow = function() {
        ...
        if (selectRow) {
            for (var i in selectRow) {
                rowList.push(selectRow[i]);
            }
        }
        return rowList;
    };
    this.endAllLine = function() {
...
com.auditDialog = function() {
    ...
    if (winAudit.length == 0) {
        ...
        html += '            <div class="grid_13 val"><textarea style="width:272px;height:60px;" id="AuditCmment" class="z-text" data-bind="value:form.comment" ></textarea></div> '
...
com.exporter = function(opt) {
    ...
    initFromGrid = function(grid) {
        ...
        if (grid.toolbar) {
            options.url = grid.url;
            options.queryParams = grid.urlParams;
...
com.exporterTemplate = function(opt) {
    var self = this;
    var dataAction = opt.dataAction == 'undefined' ? "" : opt.dataAction
    var defaultOptions = {
        action: "/home/download",
        dataGetter: "api",
    ...
    this.download = function(suffix) {
        self.params.fileType = suffix || "xls" || "xlsx";
        self.params.dataParams = JSON.stringify(self.params.dataParams);
        self.params.titles = JSON.stringify(self.params.titles);
    ...
    initFromGrid = function(grid) {
        ...
        var columnsTemplate = [];
        var columnsTitleTemplate = [];
        if (typeof (grid.excelTemplate) != 'undefined') {
            columnsTemplate = grid.excelTemplate;
        }
        if (typeof (grid.excelTemplateTitle) != 'undefined') {
            columnsTitleTemplate = grid.excelTemplateTitle;
        }
        var _titels = [];
        ...
        for (var i = 0; i < length; i++) {
            if (options.frozenColumns.length > 0) {
                for (var k = 0, x = 0; k < columnsTemplate.length, x < columnsTemplate.length; k++ , x++) {
                    for (var j = 0; j < options.columns[i].length; j++) {
                        if (columnsTemplate[k] == options.columns[i][j].field) {
                            _titels.push(options.columns[i][j]);
                        }
                    }

                    for (var m = 0; m < options.frozenColumns[i].length; m++) {
                        if (columnsTemplate[x] == options.frozenColumns[i][m].field) {
                            _titels.push(options.frozenColumns[i][m]);
                        }
                    }
                }
            }
            ...
            if (_titels.length < columnsTemplate.length) {
                _titels = [];
                for (var k = 0; k < columnsTemplate.length; k++) {
                    grid.templateObject = {};
                    grid.templateObject.field = columnsTemplate[k];
                    grid.templateObject.title = columnsTitleTemplate[k];
                    _titels.push(grid.templateObject);
                }
            }
            titles[i] = _titels;
        }
...
com.formatDate = function(value) {
    if (value) {
        ...
        return utils.formatDate(tempdate, 'yyyy-MM-dd');
    }
};
com.formatTime = function(value) {
    if (value) {
        var tempdate = new Date(value);
        return utils.formatDate(tempdate, 'yyyy-MM-dd hh:mm:ss');
    }
}
com.readonly = function(formId, isDisabled) {
    var attr = "disable";
    if (!isDisabled) {
        attr = "enable";
    }
    $("div[id='" + formId + "'] :text").attr("disabled", isDisabled);
    $("div[id='" + formId + "'] textarea").attr("disabled", isDisabled);
    $("div[id='" + formId + "'] select").attr("disabled", isDisabled);
    $("div[id='" + formId + "'] :radio").attr("disabled", isDisabled);
    $("div[id='" + formId + "'] :checkbox").attr("disabled", isDisabled);
}
