...
(function ($, ko) {
    ...
    ko.creatEasyuiValueBindings = function (o) {
        ...
        var customBinding = {
            ...
            update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                ...
                if (o.type == "combobox" && jqElement(element).attr("setOtherData")) {
                    var otherId = jqElement(element).attr("setOtherData");
                     
                    var othervalue = jqElement(element).combobox("getText");
                   
                    $(":input[filedname='" + otherId + "' ]").val(othervalue);
                    if (viewModel.pageData) {
                        viewModel.pageData.form[otherId] = othervalue;
                    } else {
                        viewModel.form[otherId] = othervalue;
                    }
    ...
    ko.creatEasyuiValueBindings({ type: 'combogrid', event: 'onSelect' });
    ...
    ko.bindingHandlers.datasource = {
        init: function (element, valueAccessor) {
            ...
            if (jq.data('treegrid'))
                ...
                var val = jq.combogrid('getValue'), ds = ds.rows || ds;
                        ...
                        var winsize = function (size) {
                            ...
                            if (size && size.w) {
                                if ($("#navigation").size() > 0) {
                                    ret.width = jqElement(window).width() - size.w - $("#navigation").width()-8;
                                } else {
                                    ret.width = jqElement(window).width() - size.w
                                }
    ...
    ko.createEasyuiInitBindings({ type: 'numberbox' });
