    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            w.find("#pwd_confirm").click(function () {
                var p1 = w.find("[name=p1]").val();
                var p2 = w.find("[name=p2]").val();
                if (p1!=p2) {
                    com.message('success', "新密码与确认密码不一样！");
                    return;
                }
                com.ajax({
                    type: 'POST',
                    url: '/api/sys/user/PostEditUserPassword/?id=' + w.find("[name=UserCode]").val()+'&pwd='+p2,
                    success: function () {
...
wrapper.setFullScreen = function () {
    ...
    if (that.find('.icon-screen_full').length) {
        ...
        centerPanel.panel('resize', { top: 103 });
...
wrapper.tabRefresh = function (url) {
    ...
    if (typeof src === 'string') {
        if (typeof url === 'string')
            $tab.tabs('update', { tab: currentTab, options: { content: wrapper.createFrame(url) } })
        else
            $tab.tabs('update', { tab: currentTab, options: { content: wrapper.createFrame(src) } })
    }
};
